...
from espnet.nets.beam_search_pilot_inc import BeamSearchPilotInc, Hypothesis
...
class BatchBeamSearchPilotInc(BeamSearchPilotInc):
    ...
    def search(self, running_hyps: List[Hypothesis], x: torch.Tensor) -> List[Hypothesis]:
        ...
        """
        running_hyps = self.batchfy(running_hyps)
    ...
    def post_process(
        ...
        if self.token_list is not None:
            for t in range(len(running_hyps.yseq)):
                logging.info(
                    "best " + str(t+1) + " hypo: "
                    + "".join(
                        [
                            self.token_list[x]
                            for x in running_hyps.yseq[t, 1 : running_hyps.length[0]]
                        ]
                    )
                )
        ...
        batch_running_hyps = self._batch_select(running_hyps, remained_ids)
        list_running_hyps = self.unbatchfy(batch_running_hyps)
        #return self._batch_select(running_hyps, remained_ids)
        return list_running_hyps
