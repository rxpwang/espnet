...
class BranchformerEncoderLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.use_two_branches:
            if merge_method == "concat":
                ...
                # linear projection after weighted average
                self.merge_proj = torch.nn.Linear(size, size)
            elif merge_method == "fixed_learned":
                if cgmlp_weight != 1.0:
                    # attention-based pooling for two branches
                    self.pooling_proj1 = torch.nn.Linear(size, 1)
                    self.pooling_proj2 = torch.nn.Linear(size, 1)
    ...
    def forward(self, x_input, mask, cache=None):
        ...
        if self.use_two_branches:
            ...
            elif (self.merge_method == "learned_ave") or (self.merge_method == "fixed_learned"):
                if (
                    self.training
                    ...
                    w1, w2 = 0.0, 1.0
                elif (self.attn_branch_drop_rate == 1.0):
                    w1, w2 = 0.0, 1.0
                else:
                    # branch1
                    score1 = (
        ...
        if self.use_two_branches:
            if self.merge_method == "concat":
                ...
                if (
                    ...
                    w1, w2 = merge_weights[:, 0], merge_weights[:, 1]  # (batch, 1, 1)
                '''
                recordname = '/u/waq9hw/Documents/Speech/espnet/egs2/slurp/slu1/weight_record.txt'
                with open(recordname, 'a') as f:
                    weight1 = torch.squeeze(w1)
                    weight1 = weight1.item()
                    weight2 = torch.squeeze(w2)
                    weight2 = weight2.item()
                    record_w = str(weight1) + ' ' + str(weight2) + '\n'
                    f.write(record_w)
                '''
                x = x + stoch_layer_coeff * self.dropout(
                    self.merge_proj(w1 * x1 + w2 * x2)
...
class BranchformerEncoder(AbsEncoder):
    ...
    def __init__(
        ...
        cgmlp_weight = [1.0] * num_blocks
        cgmlp_weight[num_blocks-1] = 0.0
...
class BranchformerEncoderInf(AbsEncoder):
    ...
    def __init__(
        ...
        print("Using Branchformer modified version!")
        #attn_branch_drop_rate = numpy.ones(9)
        #attn_branch_drop_rate = numpy.zeros(9)
        #attn_branch_drop_rate[2] = 0.0
        #attn_branch_drop_rate[7] = 1.0
    ...
    def forward(
        ...
        if (
            ...
            tmp1 = xs_pad.unsqueeze(1)
            tmp = self.embed.conv(tmp1)
            pooling_cnn = torch.nn.AvgPool3d(kernel_size=(128, 3, 2))
            tmp_o = pooling_cnn(tmp)
            k = tmp_o.size()
            if k[2] > 33:
                tmp_o = tmp_o[:, :, :33, :]
            else:
            ...
            with open(recordname, 'a') as f:
                tmp_size = tmp.size()
                tmp_size = str(tmp_size)
                record_w = tmp_size + '\n'
                f.write(record_w)
            '''
            xs_pad, masks = self.embed(xs_pad, masks)
            tmp = xs_pad[0]
            pooling_linear = torch.nn.AvgPool2d(kernel_size=(3, 64))
            tmp_o = pooling_linear(tmp)
            k = tmp_o.size()
            if k[1] > 33:
                tmp_o = tmp_o[:, :33, :]
            else:
            ...
            if os.path.exists(r'/u/waq9hw/Documents/Speech/espnet/egs2/slurp/slu1/Linear.npy'):
                ...
                tmp1 = numpy.concatenate((tmp1, tmp_o), axis=0)
                numpy.save(r'/u/waq9hw/Documents/Speech/espnet/egs2/slurp/slu1/Linear.npy', tmp1)
            else:
            ...
            with open(recordname, 'a') as f:
                tmp_size = tmp[0].size()
                tmp_size1 = tmp[1].size()
                tmp_size = str(tmp_size)
                tmp_size1 = str(tmp_size1)
                length = str(len(tmp))
                record_w = length + tmp_size + tmp_size1 + '\n'
                f.write(record_w)
            '''
        elif self.embed is not None:
            xs_pad = self.embed(xs_pad)
