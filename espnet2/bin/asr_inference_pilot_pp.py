...
from espnet.nets.batch_beam_search_pilot_inc import BatchBeamSearchPilotInc
...
from espnet.nets.beam_search_pilot_inc import BeamSearchPilotInc
...
class Speech2Text_npass:
    ...
    def __init__(
        ...
        decoder_pilot = asr_model_pilot.decoder

        ctc = CTCPrefixScorer(ctc=asr_model.ctc, eos=asr_model.eos)
        ctc_pilot = CTCPrefixScorer(ctc=asr_model_pilot.ctc, eos=asr_model_pilot.eos)

        token_list = asr_model.token_list
        token_list_pilot = asr_model_pilot.token_list
        ...
        scorers_pilot.update(
            decoder=decoder_pilot,
            ctc=ctc_pilot,
            length_bonus=LengthBonus(len(token_list)),
        )
        ...
        if asr_model.use_transducer_decoder:
                ...
                if batch_size == 1:
                    ...
                    if len(non_batch) == 0:
                        if streaming:
                            ...
                            beam_search_pilot_inc.__class__ = BatchBeamSearchPilotInc
            ...
            beam_search_pilot_inc.to(device=device, dtype=getattr(torch, dtype)).eval()
            ...
            for scorer in scorers_pilot.values():
                if isinstance(scorer, torch.nn.Module):
                    scorer.to(device=device, dtype=getattr(torch, dtype)).eval()

            logging.info(f"Beam_search: {beam_search}")
            logging.info(f"Beam_search_pilot: {beam_search_pilot}")
            logging.info(f"Beam_search_pilot_inc: {beam_search_pilot_inc}")
        ...
        self.asr_model_pilot = asr_model_pilot
        self.asr_train_args_pilot = asr_train_args_pilot
        self.converter = converter
        self.tokenizer = tokenizer
        self.beam_search = beam_search
        self.beam_search_pilot = beam_search_pilot
        self.beam_search_pilot_inc = beam_search_pilot_inc
    ...
    def __call__(
        ...
        if self.enh_s2t_task or self.multi_asr:
            ...
            # c. Passed the encoder result and the beam search
            # if data shorter than 2s, only do full decoding
            # elif data in 2 - 2 + granularity, do pilot decoding with 1.5s data then do full decoding
            # elif data longer than 2 + granularity, do pilot decoding, then loop pilot incremental decoding, then full decoding
            # last two situation can be merged into one.
            audio_sec = audio_len / 16000.0
            if audio_sec < 2:
                ...
                if isinstance(enc_partial, tuple):
                    enc_partial = enc_partial[0]
                assert len(enc_partial) == 1, len(enc_partial)
                '''
                ...
                while(partial_len + self.granularity < audio_sec):
                    enc_partial = self.partial_encoding(partial_len, speech)
                    cur_audio_len = partial_len * 16000
    ...
    def partial_encoding(self, partial_len, speech):
        # do the partial data batch producing, encoding, and enc related post processing
        batch_partial = get_partial_data_batch(partial_len, speech)
        batch_partial = to_device(batch_partial, device=self.device)
        enc_partial, _ = self.asr_model.encode(**batch_partial)
        if isinstance(enc_partial, tuple):
    ...
    def _decode_single_sample_partial(self, enc: torch.Tensor):
    ...
    def _decode_single_sample_partial_inc(self, enc: torch.Tensor, running_hyps, ended_hyps, cur_len, audio_len):
        if self.beam_search_transducer:
            logging.info("encoder output length: " + str(enc.shape[0]))
            nbest_hyps = self.beam_search_transducer(enc)
                ...
                if isinstance(self.beam_search.nn_dict.decoder, S4Decoder):
                    ...
                    for module in self.beam_search.nn_dict.decoder.modules():
                        if hasattr(module, "setup_step"):
                            module.setup_step()
            logging.info("beam_search_pilot start: {self.beam_search_pilot}")
            nbest_hyps, running_hyps, ended_hyps, cur_len = self.beam_search_pilot_inc(
    ...
    def from_pretrained(
        ...
        **kwargs: Optional[Any],
        ...
        return Speech2Text_npass(**kwargs)
...
def inference(
    ...
    speech2text_kwargs = dict(
        ...
        dtype=dtype,
        beam_size=beam_size,
        beam_size_pilot=beam_size_pilot,
    ...
    speech2text = Speech2Text_npass.from_pretrained(
        model_tag=model_tag,
        **speech2text_kwargs,
    )
...
def get_parser():
    ...
    group.add_argument("--beam_size_pilot", type=int, default=3, help="Beam size for pilot decoding")
